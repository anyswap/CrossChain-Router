...
func (b *Bridge) registerAnyCallSwapTx(txHash string, logIndex int) ([]*tokens.SwapTxInfo, []error) {
	...
	var err error
	logs, err = b.GetTransactionLog(commonInfo.Hash)
	if err != nil {
	...
	errs := make([]error, 0)
	startIndex, endIndex := 0, len(logs)
	...
	for i := startIndex; i < endIndex; i++ {
		...
		*swapInfo = *commonInfo
		...
		err := b.verifyAnyCallSwapTxLog(swapInfo, logs[i])
		switch {
...
func (b *Bridge) verifyAnyCallSwapTx(txHash string, logIndex int, allowUnstable bool) (*tokens.SwapTxInfo, error) {
	...
	err = b.verifyAnyCallSwapTxLog(swapInfo, logs[logIndex])
	if err != nil {
		return swapInfo, err
	}
