...
var (
	...
	paramCreater     string
	paramStoragePath string
	paramPublicPath  string
	paramBalancePath string
...
func main() {
	...
	registryScript, errf := ioutil.ReadFile("tokens/flow/transaction/Registry.cdc")
	if errf != nil {
		log.Fatal("ReadFile failed", "errf", errf)
	}
	script := string(registryScript)
	script = fmt.Sprintf(script, paramTokenName, paramCreater, paramStoragePath, paramPublicPath, paramBalancePath, paramTokenName)
...
func initFlags() {
	...
	flag.StringVar(&paramTokenName, "tokenName", "", "token name")
	flag.StringVar(&paramCreater, "creater", "", "token creater")
	flag.StringVar(&paramStoragePath, "storagePath", "", "token storage path")
	flag.StringVar(&paramPublicPath, "publicPath", "", "token public path")
	flag.StringVar(&paramBalancePath, "balancePath", "", "token balance path")
