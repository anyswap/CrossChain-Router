...
func initFlags() {
	...
	flag.StringVar(&poolcoin, "poolcoin", "", "poolcoin resource")
	flag.StringVar(&toAddress, "toAddress", "", "toAddress")
	flag.StringVar(&fromChainId, "fromChainId", "", "toChainId")
	flag.StringVar(&fromTx, "fromTx", "", "fromTx")
...
func main() {
	...
	tx, err := bridge.BuildSwapinTransferTransactionForScript(account.GetHexAddress(), coin, poolcoin, toAddress, strconv.FormatUint(amount, 10), fromTx, fromChainId)
	if err != nil {
		log.Fatalf("%v", err)
	}
