...
func initFlags() {
	...
	flag.StringVar(&underlying, "underlying", "", "underlying")
	flag.StringVar(&coin, "anycoin", "", "anycoin resource")
...
func main() {
	...
	tx, err := bridge.BuildWithdrawTransaction(account.GetHexAddress(), pool, underlying, coin, amount)
	if err != nil {
		log.Fatalf("%v", err)
	}
